...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($violation_stmt) {
        ...
        if ($violation_id) {
            $stmt = $conn->prepare("INSERT INTO violations (student_id, violation_id, guard_id) VALUES (?, ?, ?)");
            if ($stmt) {
    ...
    <div class="container mt-4">
        <div class="row">
            <div class="container-fluid mt-2 mb-5">
                <div class="container-fluid bg-light mt-4 p-4 rounded">
                                            ...
                                            <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <button class="btn btn-info action-btn custom-info select-student-btn" data-id="<?php echo $row['id']; ?>" data-fullname="<?php echo ucfirst($row['first_name']) . ' ' . ucfirst($row['middle_name']) . ' ' . ucfirst($row['last_name']); ?>" data-section="<?php echo ucfirst($row['section_name']); ?>"><i class="fa-solid fa-check"></i></button>
                                                    </td>
                                                </tr>
                                            <?php endwhile; ?>
                                        </tbody>
